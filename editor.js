...
var editor = function(containerID, type, save_to_url){
	...
	this.saveBlockInDB = function(block, domBlock, unlock){
		...
		if(!block.resourceID()){ // Nuevo
		...
		} /*else if(block.type == 'gallery' || block.type == 'slider') { // Update de Slider o Galería
			...
			block_data['resource_id'] = block.resourceID();
			block_data['res'] = JSON.stringify(diff);
			if (unlock === true) {
			    block_data['unlock'] = true;
			}
			$.ajax({
	            url: moduleContext.getSettings().backEndServer + "/resource/save/",
	            type: "POST",
	            data: block_data,
	            beforeSend: function(xhr, settings) {
					xhr.setRequestHeader("Authorization", "Token " + getFromLocal('content_token'));
				},
	            success: function (data) {
	                console.log('-- update gallery/slider block in DB --');
					console.log(data);
					if (unlock === true) {
					    block.locked(false);
					}
